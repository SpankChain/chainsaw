...
const saveAndCallHook = (contractEvent, eventData, callback) => {
  jsonfile.readFile(webhookFile, function(err, obj) {
    ...
    switch (contractEvent) {
      case 'DidDeposit':
        ...
        eventData = formater.eventToChannel(eventData)
        eventData['event'] = "DidDeposit"
        break;
      case 'DidSettle':
        hooks = obj['DidSettle']
        eventData['event'] = "DidSettle"
        break;
      case 'DidStartSettle':
        hooks = obj['DidStartSettle']
        eventData['event'] = formater.eventToChannel(eventData)
        eventData['event'] = "DidStartSettle"
        break;
      case 'DidCreateChannel':
        hooks = obj['DidCreateChannel']
        eventData['event'] = 'DidCreateChannel'
        break;
      case 'HubChannelUpdate':
        hooks = obj['HubChannelUpdate'];
        eventData['event'] = 'HubChannelUpdate'
        break;
      default:
        break;
      ...
      for (let i = 0; i < hooks.length; i++) {
        requestify.post(hooks[i], eventData)
          .then(function(response) {
            ...
            console.log("App server success");
...
// // Get the block every 20 seconds.
// check(){
